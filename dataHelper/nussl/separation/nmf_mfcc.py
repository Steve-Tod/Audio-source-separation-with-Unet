...
class NMF_MFCC(mask_separation_base.MaskSeparationBase):
    ...
    def run(self):
        ...
        for ch in range(n_chan):
            ...
            #print channel_stft.shape
            model=NMF(n_components=self.num_templates,init='random',random_state=0)
            channel_activation_matrix=model.fit_transform(channel_stft).T
            channel_templates_matrix=model.components_.T
            #print channel_activation_matrix.shape
            #print channel_templates_matrix.shape
            #print np.isnan(channel_stft).sum()
            # Set up NMF and run
            '''
            ...
            '''
            #channel_activation_matrix=np.nan_to_num(channel_activation_matrix)
            #channel_templates_matrix=np.nan_to_num(channel_templates_matrix)
            #print channel_templates_matrix
            #print self.mfcc_start,self.mfcc_end
            ...
            #print uncollated_masks[0]
            #print uncollated_masks[0].shape
