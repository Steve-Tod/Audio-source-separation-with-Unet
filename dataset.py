...
class TestTransformerDb(object):
    def __call__(self, sample):
        ...
        return {
            'stft': (np.abs(stft[0]), np.abs(stft[1]),
                     torch.from_numpy(
                         librosa.amplitude_to_db(np.abs(stft[2])))),
            'stft_angle': (np.angle(stft[0]), np.angle(stft[1]),
                           np.angle(stft[2])),
            'instrument':
            instrument
...
class TrainTransformer(object):
    ...
    def __call__(self, sample):
        ...
        return {
            'stft': (torch.from_numpy(np.abs(stft[0])),
...
def default_audio_loader(wav1_path, wav2_path):
    ...
    stft1 = librosa.core.stft(
        segment1, n_fft=WINDOW_SIZE, hop_length=HOP_LENGTH)
    stft2 = librosa.core.stft(
        segment2, n_fft=WINDOW_SIZE, hop_length=HOP_LENGTH)
...
class STFTData(data.Dataset):
    def __init__(self,
                 pair_dict_path,
                 root_path="/data/data_61WwmOjr/std/trainset/audios/solo/",
                 loader=default_audio_loader,
                 transform=TrainTransformerDb()):
        ...
        for k, v in self.pair_dict.items():
            ...
            for item in v:
                self.pair_info.append({
                    'instrument1':
                    instrument_dict[instrument1],
                    'instrument2':
                    instrument_dict[instrument2],
                    'wav1_path':
                    os.path.join(root_path, item['wav1_path']),
                    'wav2_path':
                    os.path.join(root_path, item['wav2_path'])
                })
    ...
    def __getitem__(self, index):
        ...
        stft1, stft2, stft_mix = self.loader(info['wav1_path'],
                                             info['wav2_path'])
