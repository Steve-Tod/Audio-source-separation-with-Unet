...
def Evaluate(jsonpath, ResultAudioPath, gtAudioPath):
    ...
    for keys in gt:
        ...
        l = len(
            read_signal(os.path.join(gtAudioPath, file_prefix + '_gt1.wav')))
        ori_ref_sources = np.zeros([2, l])
        ori_est_sources = np.zeros([2, l])
        ori_ref_sources[0, :] = read_signal(
            os.path.join(gtAudioPath, file_prefix + '_gt1.wav'))
        ori_ref_sources[1, :] = read_signal(
            os.path.join(gtAudioPath, file_prefix + '_gt2.wav'))
        ori_est_sources[0, :] = read_signal(
            os.path.join(ResultAudioPath, file_prefix + '_seg1.wav'))
        ori_est_sources[1, :] = read_signal(
            os.path.join(ResultAudioPath, file_prefix + '_seg2.wav'))
        MaxL = 222000000
