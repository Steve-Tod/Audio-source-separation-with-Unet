...
parser.add_argument('--net', '-n', default='Unet', type=str)
parser.add_argument(
    '--dataset',
    type=str)
parser.add_argument('--trainlist', default='./train_list.json', type=str)
parser.add_argument('--testlist', default='./test_list.json', type=str)
parser.add_argument('--lr', default=0.001, type=float, help='learning rate')
parser.add_argument('--bs', default=4, type=int, help='batch size')
parser.add_argument('--output_dir', default='./experiment', type=str, help='output dir')
parser.add_argument('--test_name', default=start_time, type=str, help='output dir')
parser.add_argument('--epoch', default=50, type=int, help='Number of epoch')
parser.add_argument('--log_interval', default=100, type=int, help='Steps between two log')
parser.add_argument('--load_checkpoint', default='', type=str, help='Path to checkpoint model')
...
writer = SummaryWriter(
    log_dir=log_dir, comment="Unet, 100 epochs, lr=0.001")
...
if args.load_checkpoint != '':
    trained = torch.load('./experiment/Unet_Long_Epoch_Continue/model/model19.pth')
    ...
    for i, sample in enumerate(dl, 0):
        ...
        if i % PRINT_INTERVAL == (PRINT_INTERVAL - 1):
            ...
            librosa.display.specshow(
                sample["stft"][0][3, 0, :, :].numpy(), sr=44100, hop_length=256)
            ax.set_title(instrument_array[sample['instrument'][0][3].item()])
            ax = plt.subplot(2, 3, 2)
            librosa.display.specshow(
                sample["stft"][1][3, 0, :, :].numpy(), sr=44100, hop_length=256)
            ...
            plt.subplot(2, 3, 4)
            librosa.display.specshow(
                output1.cpu().detach().numpy()[3, 0, :, :], sr=44100, hop_length=256)
            plt.subplot(2, 3, 5)
            librosa.display.specshow(
                output2.cpu().detach().numpy()[3, 0, :, :], sr=44100, hop_length=256)
            writer.add_figure('STFT_compare', fig, hist_step)
            eval_train(output1, output2, sample, sdr_list)
            
            writer.add_histogram('mask1', mask1.cpu().detach().numpy(), hist_step)
            writer.add_histogram('mask2', mask2.cpu().detach().numpy(), hist_step)
            
            # save sdr mean and std
            mean_dict, std_dict = get_sdr_dict(sdr_list, instrument_array)
            writer.add_scalars('result/sdr_mean', mean_dict, global_step=hist_step)
            writer.add_scalars('result/sdr_std', std_dict, global_step=hist_step)
            
            loss_hist.append(running_loss / PRINT_INTERVAL)
            running_loss = 0.0
            hist_step += 1
        global_step += 1
...
test_dl = data.DataLoader(stft_testset, batch_size=16, shuffle=True, num_workers=16)
model = model.eval()
